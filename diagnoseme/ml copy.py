...
def predict_disease_by_kushal2(sym):
    ...
    X = total_data.iloc[:, 0:489]  
    y = total_data.iloc[:, 489]  
...
class NeuralNetwork:
    ...
    def setup(self):
        for i in range(1, self.L):
            ...
            scale = np.sqrt(2.0 / self.layers[i-1])
            self.W[i] = tf.Variable(tf.random.normal(shape=(self.layers[i], self.layers[i-1])) * scale)
            self.b[i] = tf.Variable(tf.zeros(shape=(self.layers[i], 1)))
            self.momentum_W[i] = tf.zeros_like(self.W[i])
            self.momentum_b[i] = tf.zeros_like(self.b[i])
    ...
    def batch_norm(self, x, training=True):
        if training:
            ...
            mean = self.running_mean if hasattr(self, 'running_mean') else tf.reduce_mean(x, axis=0)
            variance = self.running_var if hasattr(self, 'running_var') else tf.reduce_mean(tf.square(x - mean), axis=0)
    ...
    def forward_pass(self, X, training=True):
        ...
        for i in range(1, self.L):
            ...
            Z = self.batch_norm(Z, training)
            
            if i != self.L - 1:
                # Use LeakyReLU for hidden layers
                A = tf.maximum(0.01 * Z, Z)
            else:
    ...
    def compute_loss(self, A, Y):
        ...
        epsilon = 0.1
        Y_smooth = (1 - epsilon) * Y + epsilon / self.num_classes
        loss = tf.nn.softmax_cross_entropy_with_logits(Y_smooth, A)
        return tf.reduce_mean(loss)

    def update_params(self, lr, beta=0.9):
        for i in range(1, self.L):
            # Update with momentum
            self.momentum_W[i] = beta * self.momentum_W[i] + (1 - beta) * self.dW[i]
            self.momentum_b[i] = beta * self.momentum_b[i] + (1 - beta) * self.db[i]
            
            self.W[i].assign_sub(lr * self.momentum_W[i])
            self.b[i].assign_sub(lr * self.momentum_b[i])
    ...
    def train_on_batch(self, X, Y, lr):
        ...
        with tf.GradientTape(persistent=True) as tape:
            A, cache = self.forward_pass(X, training=True)
            loss = self.compute_loss(A, Y)
        ...
        max_grad_norm = 5.0
        for i in range(1, self.L):
            self.dW[i] = tf.clip_by_norm(self.dW[i], max_grad_norm)
            self.db[i] = tf.clip_by_norm(self.db[i], max_grad_norm)
    ...
    def train(self, x_train, y_train, x_test, y_test, epochs, steps_per_epoch, batch_size, lr):
        ...
        for e in range(epochs):
            # Decay learning rate
            current_lr = initial_lr * (decay_rate ** (e // decay_steps))
            
            epoch_train_loss = 0.0
            print(f'Epoch {e}, Learning Rate: {current_lr:.6f}', end=' ')
            ...
            # Validation
            val_A, _ = self.forward_pass(x_test, training=False)
            val_loss = self.compute_loss(val_A, y_test).numpy()
            history['val_loss'].append(val_loss)
            ...
            print(f'Train Loss: {epoch_train_loss / steps_per_epoch:.4f}, Val Acc: {val_acc:.4f}')
            
        return history

    def predict(self, X):
        A, _ = self.forward_pass(X, training=False)
        return tf.argmax(tf.nn.softmax(A), axis=1)
